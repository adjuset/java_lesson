...
public class C50_HashMap {
	...
	public static void main(String[] args) {
		...
		System.out.println("'스파케티의 가격?' "+menus.get("스파게티"));
		System.out.println("'스테이크 가격?' "+menus.get("스테이크"));
		System.out.println("'샐러드의 가격?' "+menus.get("샐러드"));
		 
		
		//key 값 중복되면 ?
		menus.put("샐러드", 8200);	//실행결과 ? 덮어쓰기 (value)가 변경
		System.out.println(menus);
		...
		menus.remove("샐러드", 6000);			//인자 key,value : key, value가 모두 일치해야 삭제
		menus.remove("샐러드", 8200);			//인자 key,value : key, value가 모두 일치해야 삭제
